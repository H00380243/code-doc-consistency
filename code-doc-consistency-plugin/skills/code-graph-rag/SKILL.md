...
- 29 种节点类型（含 13 种 Java/Spring 专用类型）+ 44 种边类型（含 15 种 Java/Spring 专用边）
- 统一 ID 命名规范（前缀 + 相对路径 + 名称）
- 扩展字段：`confidence`、`abstraction_level`、`tentative`、`unresolved` — 用于代码-文档对比
- Java/Spring 专属字段：`annotations`、`java_package`、`java_generics`、`http_mappings`、`jpa_table`、`spring_scope` 等
...
| A.5 POM（Maven） | `parse-pom.mjs <in.json> <out.json>` | 输入 {projectRoot}，输出模块/依赖/Spring Boot 检测 |
| A.6 Spring Config | `parse-spring-config.mjs <in.json> <out.json>` | 输入 {projectRoot}，输出数据源/Redis/安全等配置 |
| B. BATCH | `compute-batches.mjs <scan.json> <imports.json> <out.json>` | 输出 batches[] + neighborMap |
| C. ANALYZE（结构） | `extract-structure.mjs <in.json> <out.json>` | 输入 {projectRoot, batchFiles[]}，输出 functions/classes/exports + Java/Spring 信息 |
...
| `code` (Java) | interfaces, enums, annotations, fields, Spring endpoints, JPA entities | `interface` + `enum` + `annotation` + `entity` + `configuration` + `endpoint` |
...
代码 tags: `entry-point`, `utility`, `api-handler`, `data-model`, `test`, `middleware`, `component`, `hook`, `service`, `type-definition`, `barrel`, `factory`, `singleton`, `event-handler`, `validation`, `serialization`, `rest-controller`, `service-layer`, `repository`, `entity`, `dto`, `configuration`, `mapper`, `security`, `cache`, `message-consumer`, `message-producer`, `grpc-service`, `feign-client`
