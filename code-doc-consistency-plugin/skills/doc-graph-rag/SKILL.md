...
```
[DISCOVER]   →   [STRUCTURED]   →   [FREETEXT × N]   →   [MERGE]   →   [REVIEW]
 文档清单         OpenAPI/UML       markdown            归一去重    schema 校验
                  /Proto/GraphQL    自由文本 LLM        多文档融合  悬挂边
                  parser            抽取
 ────确定性────  ────确定性────    ────LLM────       ────确定性────
```
...
- FREETEXT → `_workspace/02_doc_freetext_<doc>.json`
